...
namespace Tetris
{
    public partial class LevelGame : Form
    {
        ...
        public LevelGame(int UserID)
        {
            ...
            int scoreTarget = 0, allowedMoves = 0;
            canvasBlockArray = new CanvasBlock[canvasWidth, canvasHeight];
            //canvasBlockArray = LevelGenerator.GenerateLevel(25, canvasWidth, canvasHeight, out scoreTarget, out allowedMoves);

            canvasBlockArray = LevelGenerator.GenerateLevel(25, canvasWidth, canvasHeight, out scoreTarget, out allowedMoves);
            // debug messagebox of canvasblockarray
            for (int i = 0; i < canvasWidth; i++)
            {
                for (int j = 0; j < canvasHeight; j++)
                {
                    Console.Write(canvasBlockArray[i, j].IsFilled);
                    Console.Write(" ");
                }
                Console.WriteLine();
            }
        ...
        private bool CheckIfGameOver(Shape shape)
        {
            if (currentY < 0)
            {
                ...
                if (score > user.HighScore)
                {
                    ...
                    MessageBox.Show($"Game Over! New high score: {score}!");
                    this.Close();
                    timer.Stop();
                    aiTimer.Stop();
                    return false;
